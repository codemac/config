...
# -----
# LAST UPDATE:  2006/10/19 -- grande klapmuetz (archlinuxisfied)
# LAST UPDATE:  2006/06/21 -- Thomas Adam <thomas@edulinux.homeunix.org>
# -----
...
def ELinks::goto_url_hook(url, current_url)
    ...
    return current_url
    #when /n6tadam/
    #  return "http://edulinux.homeunix.org/~n6tadam/"
    end
...
def ELinks::pre_format_html_hook(url, html)
  ...
  if url.match(/http:\/\/bugs.archlinux.org.*/)
    #html.gsub!(/<script type=\"text\/javascript\" src=\"http:\/\/bugs.archlinux.org\/includes\/.*.js\"><\/script>/, '')
  end

  if url.match(/http:\/\/.*heise.de.*/)
    html.gsub!(/<!-- Site Navigation Bar -->.*<!-- Allgemeines Standard-Stylesheet -->/m, '')
    html.gsub!(/<cadv>.*<\/cadv>/m, '')
    #html.gsub!(/<!-- allgemeine obere Navigation -->.*<!-- Liste der Meldungen -->/m, '')
  end

  if url.match(/google.com/)
    html.gsub!(/<img src=\/intl\/en\/nav_page.gif width=16 height=26 alt="" border=0>/, '')
    html.gsub!(/<img src=\/intl\/en\/nav_next.gif width=100 height=26 alt="" border=0>/, '')
    html.gsub!(/<img src=\/intl\/en\/nav_previous.gif width=68 height=26 alt="" border=0>/, '')
  end

    if url.match(r)

      # Arch BBS hacks... :-)
      html.gsub!(/<\/ br>(Arch Linux Forums)/, '\1')
      html.gsub!(/<td><a href=\"index.php.*\">.*<\/td>/, '')
      html.gsub!(/<a href=\".*"><img border=0 title=\".*(Arch|FlySpray).*\".*><\/a>/, '')
        ...
        if url.match(s)
            ...
            html.gsub!(/<(img.*?)>(faq|search|memberlist|usergroups|register|profile|you have no new messages|log out|log out \[.*?\]|chat room<\/a>)/i,'\2')
